...
diff_NY <- data.frame(feature=f_to_use)
diff_NY$diff.value <- t(Kings - NY_avg)
colnames(diff_NY) <- c("feature","diff.value")

diff_NY$colour <- ifelse(diff_NY$diff.value < 0, "negative","positive")
ggplot(diff_NY,aes(feature,diff.value))+
  geom_bar(stat="identity",position="identity",aes(fill = colour))+
  scale_fill_manual(values=c(positive="firebrick1",negative="steelblue")) + coord_flip()
```
